...
class PropertyValidatorSpec extends AnyFreeSpec with Matchers {
  ...
  "exists" - {
    "should return Left(DoesNotExist) when id is None" in {
      exists[IdMonad, TestModel](fakeModel.copy(id = None), _ => Some(fakeModel).pure[IdMonad]).value shouldEqual
        EitherT.leftT[IdMonad, Unit](DoesNotExist(modelName)).value
    }
    "should return Left(DoesNotExist) when exists func returns F[None]" in {
      val id = Some(Id(1))
      exists[IdMonad, TestModel](fakeModel.copy(id = id), _ => None.pure[IdMonad]).value shouldEqual
        EitherT.leftT[IdMonad, Unit](DoesNotExist(modelName, id)).value
    }
    "should return Right(()) when exists func returns F[Some]" in {
      exists[IdMonad, TestModel](fakeModel.copy(id = Some(Id(1))), _ => Some(fakeModel).pure[IdMonad]).value shouldEqual
