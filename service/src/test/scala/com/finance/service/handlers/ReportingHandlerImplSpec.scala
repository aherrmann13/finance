...
class ReportingHandlerImplSpec extends AnyFreeSpec with Matchers with MockFactory {
  ...
  private val handler = new ReportingHandlerImpl(mockReportingService)
  ...
  "ReportingHandlerImpl" - {
    "getAccountValue" - {
      ...
      "should return CreatePaybackResponse.Ok with values from service" in {
        (mockReportingService.getAccountValue _).when(queryModel).returns(Seq(accountValueModel, accountValueModel))
    ...
    "getNetWorth" - {
      "should return GetNetWorthResponse.Ok with value from service" in {
        (mockReportingService.getNetWorth _).when().returns(Usd(1000.0))
